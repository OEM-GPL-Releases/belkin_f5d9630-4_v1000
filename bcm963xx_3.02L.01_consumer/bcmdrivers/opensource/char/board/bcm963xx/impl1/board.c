...
void kerSysSetMacAddress(char* pucaMacAddr) 
{
	...
	printk("New Mac: %02X %02X %02X %02X %02X %02X\r\n",mac[0],mac[1],mac[2],mac[3],mac[4],mac[5]);
	     ...
	     if ((pucaMacAddr[5] == 0xff) || (pucaMacAddr[5] == 0xFF)){
		 	macAddr1[0] = 0xff;
	     }
	     if ((pucaMacAddr[4] == 0xff) || (pucaMacAddr[4] == 0xFF)){
		 	macAddr1[1] = 0xff;
	     }

	     if ((macAddr1[0] == 0xff)&&(macAddr1[1] == 0xff)){
		  	pucaMacAddr[3] = pucaMacAddr[3] + 1;
	     	}
